...
public class ContactInMemoryRepository implements ContactRepository {
    ...
    public Contact getById(Long id) {
        Contact result = contacts.stream().filter(contact -> contact.getId().equals(id)).findAny().orElse(null);
        return result;
    }
    ...
    public Contact update(Contact newContact) {
        Contact currentContact = contacts.stream().filter(contact -> contact.getId().equals(newContact.getId())).findAny().orElse(null);
    ...
    public void delete(Long id) {
        Contact currentContact = contacts.stream().filter(contact -> contact.getId().equals(id)).findAny().orElse(null);
